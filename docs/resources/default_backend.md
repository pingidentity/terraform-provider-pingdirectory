...
- `insignificant_config_archive_base_dn` (Set of String) The base DN that is considered insignificant for the purpose of maintaining the configuration archive. Supported in PingDirectory product version 9.3.0.0+.
...
- `maintain_config_archive` (Boolean) Indicates whether the server should maintain the config archive with new changes to the config backend. Supported in PingDirectory product version 9.3.0.0+.
...
- `max_config_archive_count` (Number) Indicates the maximum number of previous config files to keep as part of maintaining the config archive. Supported in PingDirectory product version 9.3.0.0+.
