	...
	schemaDef := schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"enable_control_mapping": schema.BoolAttribute{
				Description: " When the `type` value is one of [`dn-mapper`]: Indicates whether DN mapping should be applied to DNs that may be present in specific controls. DN mapping will only be applied for control types which are specifically supported by the DN mapper plugin. When the `type` value is one of [`attribute-mapper`]: Indicates whether mapping should be applied to attribute types that may be present in specific controls. If enabled, attribute mapping will only be applied for control types which are specifically supported by the attribute mapper plugin.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			"always_map_responses": schema.BoolAttribute{
				Description: " When the `type` value is one of [`dn-mapper`]: Indicates whether DNs in response messages containing the target DN should always be remapped back to the source DN. If this is \"false\", then mapping will be performed for a response message only if one or more elements of the associated request are mapped. Otherwise, the mapping will be performed for all responses regardless of whether the mapping was applied to the request. When the `type` value is one of [`attribute-mapper`]: Indicates whether the target attribute in response messages should always be remapped back to the source attribute. If this is \"false\", then the mapping will be performed for a response message only if one or more elements of the associated request are mapped. Otherwise, the mapping will be performed for all responses regardless of whether the mapping was applied to the request.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			...
			"expiration_offset": schema.StringAttribute{
				Description: " When the `type` value is one of [`purge-expired-data`]: The duration to wait after the value specified in datetime-attribute (and optionally datetime-json-field) before purging the data. When the `type` value is one of [`clean-up-inactive-pingfederate-persistent-sessions`]: Sessions whose last activity timestamp is older than this offset will be removed.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			...
			"log_interval": schema.StringAttribute{
				Description: " When the `type` value is one of [`periodic-stats-logger`]: The duration between statistics collection and logging. A new line is logged to the output for each interval. Setting this value too small can have an impact on performance. When the `type` value is one of [`monitor-history`]: The duration between logging dumps of cn=monitor to a file.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			...
			"log_file": schema.StringAttribute{
				Description: " When the `type` value is one of [`periodic-stats-logger`]: The file name to use for the log files generated by the Periodic Stats Logger Plugin. The path to the file can be specified either as relative to the server root or as an absolute path. When the `type` value is one of [`monitor-history`]: The file name to use for the log files generated by the Monitor History Plugin. The path to the file can be specified either as relative to the server root or as an absolute path. When the `type` value is one of [`referential-integrity`]: Specifies the log file location where the update records are written when the plug-in is in background-mode processing.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			"log_file_permissions": schema.StringAttribute{
				Description: " When the `type` value is one of [`periodic-stats-logger`]: The UNIX permissions of the log files created by this Periodic Stats Logger Plugin. When the `type` value is one of [`monitor-history`]: The UNIX permissions of the log files created by this Monitor History Plugin.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			...
			"retention_policy": schema.SetAttribute{
				Description: " When the `type` value is one of [`periodic-stats-logger`]: The retention policy to use for the Periodic Stats Logger Plugin . When the `type` value is one of [`monitor-history`]: The retention policy to use for the Monitor History Plugin .",
				Required:    false,
				Optional:    false,
				Computed:    true,
				ElementType: types.StringType,
			...
			"included_local_entry_base_dn": schema.SetAttribute{
				Description: " When the `type` value is one of [`ping-one-pass-through-authentication`]: The base DNs for the local users whose authentication attempts may be passed through to the PingOne service. When the `type` value is one of [`pass-through-authentication`]: The base DNs for the local users whose authentication attempts may be passed through to an alternate server. When the `type` value is one of [`pluggable-pass-through-authentication`]: The base DNs for the local users whose authentication attempts may be passed through to the external authentication service.",
				Required:    false,
				Optional:    false,
				Computed:    true,
				ElementType: types.StringType,
			...
			"connection_criteria": schema.StringAttribute{
				Description: " When the `type` value is one of [`ping-one-pass-through-authentication`]: A reference to connection criteria that will be used to indicate which bind requests should be passed through to the PingOne service. When the `type` value is one of [`pass-through-authentication`]: Specifies a set of connection criteria that must match the client associated with the bind request for the bind to be passed through to an alternate server. When the `type` value is one of [`simple-to-external-bind`]: Specifies a connection criteria object that may be used to indicate the set of clients for which this plugin should be used. If a value is provided, then this plugin will only be used for requests from client connections matching this criteria. When the `type` value is one of [`delay`]: Specifies a set of connection criteria used to indicate that only operations from clients matching this criteria should be subject to the configured delay. When the `type` value is one of [`pluggable-pass-through-authentication`]: A reference to connection criteria that will be used to indicate which bind requests should be passed through to the external authentication service.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			...
			"try_local_bind": schema.BoolAttribute{
				Description: " When the `type` value is one of [`ping-one-pass-through-authentication`]: Indicates whether to attempt the bind in the local server first, or to only send it to the PingOne service. When the `type` value is one of [`pass-through-authentication`]: Indicates whether the bind attempt should first be attempted against the local server. Depending on the value of the override-local-password property, the bind attempt may then be attempted against a remote server if the local bind fails. When the `type` value is one of [`pluggable-pass-through-authentication`]: Indicates whether to attempt the bind in the local server first and only send the request to the external authentication service if the local bind attempt fails, or to only attempt the bind in the external service.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			"override_local_password": schema.BoolAttribute{
				Description: " When the `type` value is one of [`ping-one-pass-through-authentication`]: Indicates whether to attempt the authentication in the PingOne service if the local user entry includes a password. This property will only be used if try-local-bind is true. When the `type` value is one of [`pass-through-authentication`]: Indicates whether the bind attempt should be attempted against a remote server in the event that the local bind fails but the local password is present. When the `type` value is one of [`pluggable-pass-through-authentication`]: Indicates whether to attempt the authentication in the external service if the local user entry includes a password. This property will be ignored if try-local-bind is false.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			"update_local_password": schema.BoolAttribute{
				Description: " When the `type` value is one of [`ping-one-pass-through-authentication`]: Indicates whether to overwrite the user's local password if the local bind fails but the authentication attempt succeeds when attempted in the PingOne service. When the `type` value is one of [`pass-through-authentication`]: Indicates whether the local password value should be updated to the value used in the bind request in the event that the local bind fails but the remote bind succeeds. When the `type` value is one of [`pluggable-pass-through-authentication`]: Indicates whether to overwrite the user's local password if the local bind fails but the authentication attempt succeeds when attempted in the external service. This property may only be set to true if try-local-bind is also true.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			"update_local_password_dn": schema.StringAttribute{
				Description: " When the `type` value is one of [`ping-one-pass-through-authentication`]: This is the DN of the user that will be used to overwrite the user's local password if update-local-password is set. The DN put here should be added to 'ignore-changes-by-dn' in the appropriate Sync Source. When the `type` value is one of [`pluggable-pass-through-authentication`]: The DN of the authorization identity that will be used when updating the user's local password if update-local-password is true. This is primarily intended for use if the Data Sync Server will be used to synchronize passwords between the local server and the external service, and in that case, the DN used here should also be added to the ignore-changes-by-dn property in the appropriate Sync Source object in the Data Sync Server configuration.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			"allow_lax_pass_through_authentication_passwords": schema.BoolAttribute{
				Description: " When the `type` value is one of [`ping-one-pass-through-authentication`]: Indicates whether to overwrite the user's local password even if the password used to authenticate to the PingOne service would have failed validation if the user attempted to set it directly. When the `type` value is one of [`pass-through-authentication`]: Indicates whether updates to the local password value should accept passwords that do not meet password policy constraints. When the `type` value is one of [`pluggable-pass-through-authentication`]: Indicates whether to overwrite the user's local password even if the password used to authenticate to the external service would have failed validation if the user attempted to set it directly.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			"ignored_password_policy_state_error_condition": schema.SetAttribute{
				Description: " When the `type` value is one of [`ping-one-pass-through-authentication`]: A set of password policy state error conditions that should not be enforced when authentication succeeds when attempted in the PingOne service. This option can only be used if try-local-bind is true. When the `type` value is one of [`pluggable-pass-through-authentication`]: A set of password policy state error conditions that should not be enforced when authentication succeeds when attempted in the external service. This option can only be used if try-local-bind is true.",
				Required:    false,
				Optional:    false,
				Computed:    true,
				ElementType: types.StringType,
			...
			"attribute_type": schema.SetAttribute{
				Description: " When the `type` value is one of [`seven-bit-clean`]: Specifies the name or OID of an attribute type for which values should be checked to ensure that they are 7-bit clean. When the `type` value is one of [`encrypt-attribute-values`]: The attribute types whose values should be encrypted. When the `type` value is one of [`composed-attribute`]: The name or OID of the attribute type for which values are to be generated. When the `type` value is one of [`referential-integrity`]: Specifies the attribute types for which referential integrity is to be maintained.",
				Required:    false,
				Optional:    false,
				Computed:    true,
				ElementType: types.StringType,
			...
			"filter": schema.SetAttribute{
				Description: " When the `type` value is one of [`modifiable-password-policy-state`]: A filter that may be used to identify entries that should support the ds-pwp-modifiable-state-json operational attribute. When the `type` value is one of [`search-shutdown`]: The filter to use for the search. When the `type` value is one of [`purge-expired-data`]: Only entries that match this LDAP filter will be eligible for having data purged. When the `type` value is one of [`unique-attribute`]: Specifies the search filter to apply to determine if attribute uniqueness is enforced for the matching entries.",
				Required:    false,
				Optional:    false,
				Computed:    true,
				ElementType: types.StringType,
			...
			"base_dn": schema.SetAttribute{
				Description: " When the `type` value is one of [`internal-search-rate`]: Specifies the base DN to use for the searches to perform. When the `type` value is one of [`modifiable-password-policy-state`]: A base DN that may be used to identify entries that should support the ds-pwp-modifiable-state-json operational attribute. When the `type` value is one of [`seven-bit-clean`]: Specifies the base DN below which the checking is performed. When the `type` value is one of [`search-shutdown`]: The base DN to use for the search. When the `type` value is one of [`referral-on-update`]: Specifies a base DN for requests for which to send referrals in response to update operations. When the `type` value is one of [`referential-integrity`]: Specifies the base DN that limits the scope within which referential integrity is maintained. When the `type` value is one of [`unique-attribute`]: Specifies a base DN within which the attribute must be unique. When the `type` value is one of [`clean-up-expired-pingfederate-persistent-access-grants`, `purge-expired-data`, `clean-up-inactive-pingfederate-persistent-sessions`, `clean-up-expired-pingfederate-persistent-sessions`]: Only entries located within the subtree specified by this base DN are eligible for purging.",
				Required:    false,
				Optional:    false,
				Computed:    true,
				ElementType: types.StringType,
			...
			"collection_interval": schema.StringAttribute{
				Description: " When the `type` value is one of [`stats-collector`]: Some of the calculated statistics, such as the average and maximum queue sizes, can use multiple samples within a log interval. This value controls how often samples are gathered, and setting this value too small can have an adverse impact on performance. When the `type` value is one of [`periodic-stats-logger`]: Some of the calculated statistics, such as the average and maximum queue sizes, can use multiple samples within a log interval. This value controls how often samples are gathered. It should be a multiple of the log-interval.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
			...
			"request_criteria": schema.StringAttribute{
				Description: " When the `type` value is one of [`last-access-time`]: Specifies a set of request criteria that may be used to indicate whether to apply access time updates for the associated operation. When the `type` value is one of [`ping-one-pass-through-authentication`]: A reference to request criteria that will be used to indicate which bind requests should be passed through to the PingOne service. When the `type` value is one of [`sub-operation-timing`]: Specifies a set of request criteria used to indicate that only operations for requests matching this criteria should be counted when aggregating timing data. When the `type` value is one of [`third-party`]: Specifies a set of request criteria that may be used to indicate that this Third Party Plugin should only be invoked for operations in which the associated request matches this criteria. When the `type` value is one of [`pass-through-authentication`]: Specifies a set of request criteria that must match the bind request for the bind to be passed through to an alternate server. When the `type` value is one of [`simple-to-external-bind`]: Specifies a request criteria object that may be used to indicate the set of requests for which this plugin should be used. If a value is provided, then this plugin will only be used for bind requests matching this criteria. When the `type` value is one of [`coalesce-modifications`]: A reference to request criteria that indicates which modify requests should be coalesced. When the `type` value is one of [`delay`]: Specifies a set of request criteria used to indicate that only operations for requests matching this criteria should be subject to the configured delay. When the `type` value is one of [`groovy-scripted`]: Specifies a set of request criteria that may be used to indicate that this Groovy Scripted Plugin should only be invoked for operations in which the associated request matches this criteria. When the `type` value is one of [`pluggable-pass-through-authentication`]: A reference to request criteria that will be used to indicate which bind requests should be passed through to the external authentication service.",
				Required:    false,
				Optional:    false,
				Computed:    true,
			},
