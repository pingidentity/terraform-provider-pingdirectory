...
type defaultObscuredValueResourceModel struct {
	Id              types.String `tfsdk:"id"`
	LastUpdated     types.String `tfsdk:"last_updated"`
	Notifications   types.Set    `tfsdk:"notifications"`
	RequiredActions types.Set    `tfsdk:"required_actions"`
...
// Read a ObscuredValueResponse object into the model struct
func readObscuredValueResponseDefault(ctx context.Context, r *client.ObscuredValueResponse, state *defaultObscuredValueResourceModel, expectedValues *defaultObscuredValueResourceModel, diagnostics *diag.Diagnostics) {
	state.Id = types.StringValue(r.Id)
	state.Description = internaltypes.StringTypeOrNil(r.Description, internaltypes.IsEmptyString(expectedValues.Description))
	// Obscured values aren't returned from the PD Configuration API - just use the expected value
	state.ObscuredValue = expectedValues.ObscuredValue
...
// Create any update operations necessary to make the state match the plan
func createObscuredValueOperationsDefault(plan defaultObscuredValueResourceModel, state defaultObscuredValueResourceModel) []client.Operation {
	var ops []client.Operation
	operations.AddStringOperationIfNecessary(&ops, plan.Description, state.Description, "description")
	operations.AddStringOperationIfNecessary(&ops, plan.ObscuredValue, state.ObscuredValue, "obscured-value")
	return ops
...
func (r *defaultObscuredValueResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	// Retrieve values from plan
	var plan defaultObscuredValueResourceModel
	diags := req.Plan.Get(ctx, &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	...
	var state defaultObscuredValueResourceModel
	readObscuredValueResponseDefault(ctx, readResponse, &state, &state, &resp.Diagnostics)
	...
	ops := createObscuredValueOperationsDefault(plan, state)
	if len(ops) > 0 {
		...
		// Read the response
		readObscuredValueResponseDefault(ctx, updateResponse, &state, &plan, &resp.Diagnostics)
...
func (r *obscuredValueResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	updateRequest := r.apiClient.ObscuredValueApi.UpdateObscuredValue(
